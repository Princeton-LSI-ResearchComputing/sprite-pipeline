...
# Shared
TRIM = expand(
    out_dir + "workup/trimmed/{sample}_{read}.fq.gz",
    sample=ALL_SAMPLES,
    read=["R1_val_1", "R2_val_2"],
)
TRIM_LOG = expand(
    out_dir + "workup/trimmed/{sample}_{read}.fastq.gz_trimming_report.txt",
    sample=ALL_SAMPLES,
    read=["R1", "R2"],
)
LE_LOG_ALL = [out_dir + "workup/ligation_efficiency.txt"]
TRIM_RD = expand(
    out_dir + "workup/trimmed/{sample}_R1.barcoded.RDtrim.fastq.gz", sample=ALL_SAMPLES
)
MASKED = expand(
    out_dir + "workup/alignments/{sample}.DNA.chr.masked.bam", sample=ALL_SAMPLES
)
...
BARCODE_FULL = expand(
    out_dir + "workup/fastqs/{sample}_R1.barcoded_full.fastq.gz", sample=ALL_SAMPLES
)

Bt2_DNA_ALIGN = expand(
    out_dir + "workup/alignments/{sample}.DNA.bowtie2.mapq20.bam", sample=ALL_SAMPLES
)
# DNA-DNA
BARCODEID = expand(
    out_dir + "workup/fastqs/{sample}_{read}.barcoded.fastq.gz",
    sample=ALL_SAMPLES,
    read=["R1", "R2"],
)
